            ...
            <div className="p-6 md:p-8">
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {featured.results.slice(0, 4).map((result) => (
                  <div key={`${result.value}-${result.metric}`} className="rounded-xl border border-gray-200 bg-white p-4">
                    <div className="text-lg font-semibold text-gray-900">{result.value}</div>
                    <div className="mt-1 text-xs font-semibold text-gray-600">{result.metric}</div>
                  </div>
                ))}
              </div>
